...
import {modules,can,canRoute,level} from './permissions.js';
let observer=null;
...
export function accessAllowed(user,ns,root){
 ...
 if(user[ns+'_access']==='none'||!home&&!canRoute(user,ns,route)){root.innerHTML='<section class="access-denied"><h1>Bu ekrana yetkin yok.</h1><p>'+esc(user.name)+', yöneticin bu ekranı hesabına açmamış.</p><a class="primary" href="'+(user[ns+'_access']==='none'?'/':ns==='ec'?'/eticaret/#overview':'/uretim/#dashboard')+'">Yetkili ekranlarıma dön</a><button class="text-button" id="access-logout">Çıkış yap</button></section>';root.querySelector('#access-logout').onclick=async()=>{await fetch('/api/auth/logout',{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});location.reload();};return false;}
 const readonly=!canRoute(user,ns,route,true);
 const apply=()=>{
  document.querySelectorAll('nav a[href="#settings"],nav a[href="#integrations"]').forEach(el=>{el.hidden=true;});
  document.querySelectorAll('a[href^="#"]').forEach(el=>{const key=el.getAttribute('href').slice(1).split('?')[0];if(!['overview','dashboard'].includes(key)&&!canRoute(user,ns,key))el.hidden=true;});

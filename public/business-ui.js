...
export function mountBusiness(root, namespace, view) {
  ...
  const state = {data:null, tab:view === 'pricing' ? 'quote' : 'parties', party:'', search:'', quote:null, quoteInput:{}, disposed:false, sequence:0};
  ...
  async function submit(event) {
    ...
    try {
      ...
      if (kind === 'quote') {
        ...
        if (!state.disposed) render(); return;
      }
      let path, body = {...x};
      if (kind === 'profile') {

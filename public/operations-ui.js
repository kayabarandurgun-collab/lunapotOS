...
const kinds={orders:'Siparişler',sale:'Satış finans kayıtları',return:'İade finans kayıtları',deductions:'Kesinti faturaları',payments:'Hakediş kayıtları',finance:'Muhasebe hareketleri',commissions:'Güncel komisyon bilgisi',delivered:'Teslim edilen paketler',shipped:'Kargoya verilen paketler',undelivered:'Teslim edilemeyen paketler'};
...
// SENKRON FORMU → UÇ GÖVDESİ. Form gövdesi her alanı METİN taşır (Object.fromEntries(new FormData())
// işaret kutusunu 'on' yapar, sayıyı '0' yapar). Sunucu 'preview' alanında yalnız GERÇEK boolean
// kabul eder ve belirsiz değeri 400 ile reddeder: yazma modu tahminle seçilirse geri alınamaz iş
// üretir (taslak açılır, imleç ilerler). Dönüştürme tek yerde toplandı ki bir dal unutulmasın.
export const senkronGirdisi=(x,onizleme=false)=>({...x,page:Number(x.page),skus:String(x.skus??'').split(',').map(s=>s.trim()).filter(Boolean),
 // Önizleme tek sayfadır: yazmayan çağrı imleci ilerletmediği için sayfa sayfa alma akışına girmez.
 all_pages:onizleme===true?false:x.all_pages==='on',preview:onizleme===true});
// "Yazmadan dene" sonucunun Türkçe özeti. Sıfır sayılar yazılmaz; cümle her zaman "hiçbir şey
// yazılmadı" ile biter çünkü kullanıcının asıl sorusu budur.
export const onizlemeOzeti=r=>'Yazmadan deneme · '+(Number(r.page)+1)+'. sayfa: '+[(r.records?.length||0)+' kaynak kaydı okundu',
 r.deliveredMarked&&r.deliveredMarked+' paket teslim işaretlenecek',r.alreadyKnownOrders&&r.alreadyKnownOrders+' sipariş zaten sistemde',
 r.importableOrders&&r.importableOrders+' yeni taslak açılacak',r.changed&&r.changed+' kayıt değişmiş',r.unchanged&&r.unchanged+' kayıt aynı',
 r.ambiguousDeliveries&&r.ambiguousDeliveries+' teslim elle onay bekliyor',r.undatedDeliveries&&r.undatedDeliveries+' teslimde tarih gelmedi'
].filter(Boolean).join(' · ')+' · hiçbir şey yazılmadı.'+(r.hasMore?' Bu tarih aralığında başka sayfalar da var; gerçek alımda sırayla çekilir.':'')
...
export function mountOperations(root,namespace,view){
 ...
 // 'ek' düğmeleri aynı formu gönderir; data-op TAŞIMAZLAR çünkü kök tıklama dinleyicisi [data-op]
 // arıyor ve gönderme akışı ile tıklama akışı birbirine karışmamalı. Ayırt etme data-mode ile olur.
 function dialog(title,action,body,ek=''){close();root.insertAdjacentHTML('beforeend','<dialog class="ins-dialog" aria-labelledby="op-dialog-title"><form data-op-form="'+action+'"><div class="dialog-heading"><h2 id="op-dialog-title">'+title+'</h2><button class="icon-button" type="button" data-op="close" aria-label="Kapat">×</button></div><div class="form-body">'+body+'<p class="error" id="op-form-error" role="alert"></p></div><div class="dialog-footer"><button class="secondary" type="button" data-op="close">Vazgeç</button>'+ek+'<button class="primary" type="submit">Devam et</button></div></form></dialog>');$('dialog').showModal();}
 async function load(){
 ...
 root.addEventListener('click',async e=>{const b=e.target.closest('[data-op]');if(!b)return;const action=b.dataset.op,provider=b.dataset.provider;if(action==='close'){close();return;}if(action==='stop-sync'){syncAbort?.abort();return;}if(busy)return;try{
  ...
  if(action==='sync'){const p=state.providers.find(p=>p.id===provider);dialog(names[provider]+' · Verileri al','sync',field('Sağlayıcı','provider',provider,'hidden')+select('Alınacak kayıtlar','kind',p.capabilities.map(k=>[k,kinds[k]||k]))+'<div class="field-grid">'+field('Başlangıç','from',provider==='hepsiburada'?date():new Date(Date.now()-6*86400000).toISOString().slice(0,10),'date','required')+field('Bitiş','to',date(),'date','required')+'</div>'+(provider==='hepsiburada'?'<p class="help">Hepsiburada tek seferde <strong>en fazla bir gün</strong> veriyor: başlangıç ve bitiş aynı gün olmalı. Paket durumları (teslim edilen, kargoya verilen, teslim edilemeyen) yalnız <strong>son bir ay</strong> için alınabilir. “Siparişler” ucu sadece paketlenmeyi bekleyenleri döndürür; hepsini paketlediysen boş gelmesi normaldir.</p>':'')+field('Başlangıç sayfası · ilk alımda 0','page',0,'number','required min="0" max="10000" step="1"')+field('HB komisyon SKU kodları (virgülle ayır)','skus','','text','maxlength="4000"')+'<label><input type="checkbox" name="all_pages" checked> Seçilen tarih aralığının tüm sayfalarını sırayla al</label><p class="help">İlerlemeyi izleyebilir ve duraklatabilirsin. Sayfa açıkken çalışır; aynı kayıtlar tekrar alınırsa çoğaltılmaz. Bağlantı hatasızsa sunucu birkaç saatte bir kendisi de çeker.</p><p class="help">Ne olacağını önce görmek istersen <strong>Yazmadan dene</strong>: seçilen sayfa alınır, sayılar gösterilir, hiçbir şey kaydedilmez.</p>',
   '<button class="secondary" type="submit" data-mode="preview">Yazmadan dene</button>');return;}
 ...
 async function runSync(input){
  // YAZMADAN DENE: tek sayfa, tek istek. Önizleme imleç yazmadığı için sayfa sayfa alma akışına
  // (pullSourcePages) girmez; duraklatma/devam düğmesi de anlamsızdır, geri alınacak bir iş yok.
  // Ekran yenilenmez: hiçbir kayıt değişmedi, yenilemek boşuna istek olurdu.
  if(input.preview===true){close();notice(onizlemeOzeti(await api('/connections/'+input.provider+'/sync',input)));return;}

...
export function level(user,ns,key){if(user?.owner)return 'write';const envelope=user?.[ns+'_access']||'none';if(!modules[ns]?.[key])return 'none';
 const stored=user?.permissions?.[ns];if(key==='webshop'&&!stored?.webshop)return 'none';const legacy=key==='amounts'&&stored&&!Object.hasOwn(stored,'amounts');

...
export function productList(products,data,state,helpers){
 ...
 const detailRows=p=>'<div><dt>Eldeki / ayrılan</dt><dd>'+qty(p.quantity_milli)+' / '+qty(p.reserved_milli||0)+'</dd></div><div><dt>Ort. alış fiyatı</dt><dd>'+money(gross(p,p.average_purchase_cents))+tag(p)+'</dd></div><div><dt>Ort. satış fiyatı</dt><dd>'+money(gross(p,price(p)))+tag(p)+'</dd></div>'+(data.productStats?'<div><dt>Satılan</dt><dd>'+(st(p)?qty(st(p).adet_milli):'0')+' '+esc(p.stock_unit)+'</dd></div><div><dt>Ciro</dt><dd>'+(st(p)?money(st(p).ciro_cents):'—')+'</dd></div><div><dt>Toplam kâr</dt><dd>'+(st(p)?kar(st(p).kar_cents)+tahminNot(p):'—')+'</dd></div><div><dt>Adet başı kâr</dt><dd>'+(st(p)?kar(st(p).kar_adet_cents):'—')+'</dd></div>':'');
 const tableRows=list=>list.map(p=>'<tr><td class="product-table-name"><strong>'+esc(p.name)+'</strong><small>'+esc(p.sku)+' · '+esc(supplier(p))+'</small></td><td><strong class="'+(available(p)<0?'ol-neg':'')+'">'+qty(available(p))+' '+esc(p.stock_unit)+'</strong><small>Eldeki '+qty(p.quantity_milli)+' · Ayrılan '+qty(p.reserved_milli||0)+'</small></td><td>'+money(gross(p,cost(p)))+tag(p)+'</td><td>'+money(gross(p,p.value_cents))+tag(p)+'</td><td><details class="product-details"><summary>Maliyet ve satış</summary><dl><div><dt>Kategori</dt><dd>'+esc(p.category||'Kategorisiz')+'</dd></div>'+detailRows(p)+'</dl></details></td><td>'+actions(p)+'</td></tr>').join('');
 ...
 const tile=helpers.compact?p=>'<article class="card product-tile"><div class="product-eyebrow"><span>'+esc(p.category||'Kategorisiz')+'</span><span class="pill '+(available(p)<=p.min_stock_milli?'warning':'success')+'">'+status(p)+'</span></div><h3>'+esc(p.name)+'</h3><p class="product-code">'+esc(p.sku)+'</p><p class="product-supplier">'+esc(supplier(p))+'</p><div class="product-available '+(available(p)<0?'ol-neg':'')+'"><span>Kullanılabilir stok</span><strong>'+qty(available(p))+' <small>'+esc(p.stock_unit)+'</small></strong></div><dl class="product-key-values"><div><dt>Birim stok maliyeti</dt><dd>'+money(gross(p,cost(p)))+tag(p)+'</dd></div><div><dt>Stok değeri</dt><dd>'+money(gross(p,p.value_cents))+tag(p)+'</dd></div></dl><details class="product-details"><summary>Maliyet ve satış ayrıntıları</summary><dl>'+detailRows(p)+'</dl></details><div class="product-actions">'+actions(p)+'</div></article>':p=>'<article class="card product-tile"><div class="product-eyebrow"><span>'+esc(p.category||'Kategorisiz')+'</span><span class="pill '+(p.quantity_milli<=0?'warning':'neutral')+'">'+status(p)+'</span></div><h2>'+esc(p.name)+'</h2><p class="muted">'+esc(p.sku)+'</p><p class="product-supplier">'+esc(supplier(p))+'</p><dl><div><dt>Kullanılabilir</dt><dd>'+qty(available(p))+' '+esc(p.stock_unit)+'</dd></div><div><dt>Eldeki / ayrılan</dt><dd>'+qty(p.quantity_milli)+' / '+qty(p.reserved_milli||0)+'</dd></div><div><dt>Ort. alış fiyatı</dt><dd>'+money(gross(p,p.average_purchase_cents))+tag(p)+'</dd></div><div><dt>Birim stok maliyeti</dt><dd>'+money(gross(p,cost(p)))+tag(p)+'</dd></div><div><dt>Ort. satış fiyatı</dt><dd>'+money(gross(p,price(p)))+tag(p)+'</dd></div><div><dt>Stok değeri</dt><dd>'+money(gross(p,p.value_cents))+tag(p)+'</dd></div>'+(data.productStats?'</dl><dl class="product-sales"><div><dt>Satılan</dt><dd>'+(st(p)?qty(st(p).adet_milli):'0')+' '+esc(p.stock_unit)+'</dd></div><div><dt>Ciro</dt><dd>'+(st(p)?money(st(p).ciro_cents):'—')+'</dd></div><div><dt>Toplam kâr</dt><dd>'+(st(p)?kar(st(p).kar_cents)+tahminNot(p):'—')+'</dd></div><div><dt>Adet başı kâr</dt><dd>'+(st(p)?kar(st(p).kar_adet_cents):'—')+'</dd></div>':'')+'</dl><div class="product-actions">'+actions(p)+'</div></article>';

...
const money=v=>v==null?'Eksik veri':new Intl.NumberFormat('tr-TR',{style:'currency',currency:'TRY'}).format(v/100);
...
const channelNames={trendyol:"Trendyol",hepsiburada:"Hepsiburada",purchase:"Alış eşleşmesi",other:"Diğer"};
// Baglantilar salt okunur gosterilir; eslestirme surumleme veya stok modeli degistirilmez.
export function linksBlock(state){
 const links=state?.links;if(!Array.isArray(links)||!links.length)return "";
 const unit=state.product?.stock_unit||"";
 const row=l=>{
  const per=number(l.quantity_milli);
  const set=l.component_count>1?` · set: 1 satışta ${per} ${esc(unit)} düşer, 3 satışta ${number(l.quantity_milli*3)} ${esc(unit)}`:` · 1 satışta ${per} ${esc(unit)}`;
  const who=l.source==="purchase"?(l.supplier_name?" — "+esc(l.supplier_name):" — tedarikçi adı yok"):"";
  return `<li><strong>${esc(channelNames[l.source]||l.source)}</strong> · ${esc(l.external_code)}${who}<small>${set}</small></li>`;
 };
 const listings=links.filter(l=>l.source!=="purchase"),purchases=links.filter(l=>l.source==="purchase");
 return `<details class="history-links"><summary>Bağlı ilanlar, setler ve tedarikçi kodları (${links.length})</summary>`+
  (listings.length?`<p class="help">Satış ilanları</p><ul>${listings.map(row).join("")}</ul>`:"")+
  (purchases.length?`<p class="help">Alış / tedarikçi kodları</p><ul>${purchases.map(row).join("")}</ul>`:"")+
  (state.links_truncated?`<p class="help">İlk 50 bağlantı gösteriliyor.</p>`:"")+"</details>";
}
export function openStockHistory(root,namespace,productId,parentSignal){
 ...
 const d=document.createElement('dialog');d.className='stock-history-dialog workflow-dialog';prepareWorkflow(d);d.setAttribute('aria-labelledby','stock-history-title');root.append(d);

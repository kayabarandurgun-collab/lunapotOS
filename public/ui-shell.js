...
function enhance(){scheduled=false;for(const busy of document.querySelectorAll("[aria-busy=true]"))if(!busy.disabled)busy.removeAttribute("aria-busy");enhanceNavigationSearch();enhanceLists();const next=document.querySelector('#sidebar');if(next!==sidebar){navObserver?.disconnect();sidebar=next;if(sidebar){navObserver=new MutationObserver(syncMenu);navObserver.observe(sidebar,{attributes:true,attributeFilter:['class']});}}syncMenu();

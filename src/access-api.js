...
export async function currentSession(request,db){const token=request.headers.get('Cookie')?.match(/(?:^|; )lunapot_session=([a-f0-9]{64})(?:;|$)/)?.[1];if(!token)return null;
 const row=await db.prepare('SELECT s.token_hash,s.staff_id,u.name,u.ec_access,u.lp_access,u.active,u.password_hash FROM sessions s LEFT JOIN staff_users u ON u.id=s.staff_id WHERE s.token_hash=? AND s.expires_at>?').bind(await hash(token),Math.floor(Date.now()/1000)).first();
 if(!row||row.staff_id&&(!row.active||!row.password_hash))return null;
 return {token_hash:row.token_hash,user:row.staff_id?{id:row.staff_id,name:row.name,owner:false,ec_access:row.ec_access,lp_access:row.lp_access}:owner};
}
export function authorize(user,path,method){
 if(user.owner||path==='/api/auth/logout')return;
 if(path.startsWith('/api/admin/')||path==='/api/admin')fail('Bu ekran yalnızca yöneticiye açık.',403);
 const match=path.match(/^\/api\/(ec|lp)(\/.*)?$/),ns=match?.[1]||'lp',sub=match?.[2]||'';
 const access=user[ns+'_access'];if(access==='none')fail('Bu çalışma alanına erişim yetkiniz yok.',403);
 const write=!['GET','HEAD'].includes(method);
 if(write&&access!=='write')fail('Bu çalışma alanında yalnızca görüntüleme yetkiniz var.',403);
 if(sub.startsWith('/settings/backup')||sub.startsWith('/recovery')||write&&/^\/(settings|connections|integrations)(\/|$)/.test(sub))fail('Bağlantı, yedek ve şirket ayarlarını yalnızca yönetici değiştirebilir.',403);
}
...
export async function accessApi(request,env,path,readBody,user){
 ...
 if(request.method==='GET'&&!match[1]){const result=await db.batch([db.prepare('SELECT id,username,name,ec_access,lp_access,active,password_hash IS NOT NULL activated,invite_expires_at,created_at,updated_at FROM staff_users ORDER BY name'),db.prepare('SELECT * FROM access_audit ORDER BY created_at DESC,rowid DESC LIMIT 100')]);return {users:result[0].results,audit:result[1].results};}
 ...
 if(!match[1]||match[2]==='invite'){
  ...
  if(!match[1]){const username=field(x.username,'Kullanıcı adı',60).toLowerCase();if(!/^[a-z0-9][a-z0-9._-]{2,59}$/.test(username)||username==='admin'||username==='owner')fail('Kullanıcı adı 3–60 harf/rakam içermeli; admin ve owner ayrılmıştır.');
   ...
   items.push(db.prepare('INSERT INTO staff_users(id,username,name,ec_access,lp_access,invite_hash,invite_expires_at) VALUES(?,?,?,?,?,?,?)').bind(id,username,field(x.name,'Ad',100),x.ec_access,x.lp_access,await hash(token),expires));
 ...
 await db.batch([db.prepare('UPDATE staff_users SET name=?,ec_access=?,lp_access=?,active=?,updated_at=CURRENT_TIMESTAMP WHERE id=?').bind(field(x.name,'Ad'),x.ec_access,x.lp_access,x.active?1:0,id),log(db,user,'Çalışan yetkisi güncellendi; eski oturumları kapatıldı',id)]);return {id};
}

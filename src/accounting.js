...
export async function accountingApi(request,env,path,readBody){
 ...
 if(path==='/api/accounting/products'&&method==='POST'&&env.WORKSPACE==='ec'){
  const x=await readBody(request),key=id();if(x.supplier_id&&!await statement(db,"SELECT id FROM suppliers WHERE id=? AND kind='supplier'",[x.supplier_id]).first())fail('Ana tedarikçi bu çalışma alanında bulunamadı.',404);if(!['adet','kg','g','L','ml'].includes(x.stock_unit))fail('Stok birimi geçersiz.');
  await batch(db,[statement(db,'INSERT INTO products(id,name,sku,category,stock_unit,min_stock_milli,brand,supplier_id) VALUES(?,?,?,?,?,?,?,?)',[key,text(x.name,'Ürün adı'),text(x.sku,'Ürün kodu'),optional(x.category).slice(0,100),x.stock_unit,x.min_stock===0?0:milli(x.min_stock),optional(x.brand).slice(0,100),x.supplier_id||null]),log(db,'E-ticaret ürünü eklendi')]);return {id:key};
 }
 const productMatch=path.match(/^\/api\/accounting\/products\/([\w-]+)$/);
 if(productMatch&&method==='POST'&&env.WORKSPACE==='ec'){
  const x=await readBody(request),key=productMatch[1],old=await statement(db,'SELECT * FROM products WHERE id=?',[key]).first();if(!old)fail('Ürün bulunamadı.',404);if(x.supplier_id&&!await statement(db,"SELECT id FROM suppliers WHERE id=? AND kind='supplier'",[x.supplier_id]).first())fail('Ana tedarikçi bu çalışma alanında bulunamadı.',404);
...
const invoiceMatch=path.match(/^\/api\/accounting\/invoices\/([\w-]+)(?:\/(post|cancel|receive))?$/);
 if(invoiceMatch&&method==='GET'){
  ...
  const adjustments=env.WORKSPACE==='ec'?(await statement(db,'SELECT a.*,(SELECT id FROM purchase_adjustments r WHERE r.reversal_of=a.id) reversed_by FROM purchase_adjustments a JOIN purchase_lines l ON l.id=a.line_id WHERE l.invoice_id=? ORDER BY a.created_at,a.rowid',[invoice.id]).all()).results:[];
  // Özgün belge: çok faturalı PDF'te bu faturanın sayfası, tek faturalı belgede belgenin kendisi.
  const document_=await statement(db,"SELECT d.id,d.filename,d.chunk_count,d.page_count,p.page_no FROM purchase_document_pages p JOIN purchase_documents d ON d.id=p.document_id WHERE p.invoice_id=? AND d.status!='receiving' UNION ALL SELECT d.id,d.filename,d.chunk_count,d.page_count,NULL FROM purchase_documents d WHERE d.invoice_id=? AND d.status!='receiving' LIMIT 1",[invoice.id,invoice.id]).first();
  return {...invoice,document:document_||null,returns,adjustments,receipt_reversals:reversals,splits:(await statement(db,'SELECT * FROM purchase_line_splits WHERE invoice_id=? ORDER BY created_at',[invoice.id]).all()).results.map(s=>({...s,original:JSON.parse(s.original_json),allocations:JSON.parse(s.allocations_json)})),lines:(await statement(db,`SELECT l.*,${env.WORKSPACE==='ec'?'COALESCE((SELECT cancelled_milli FROM purchase_line_limits WHERE id=l.id),0)':'0'} cancelled_milli,COALESCE((SELECT SUM(quantity_milli) FROM ${receiptTable} WHERE line_id=l.id),0) received_milli FROM purchase_lines l WHERE invoice_id=? ORDER BY rowid`,[invoice.id]).all()).results,receipts:(await statement(db,'SELECT r.* FROM goods_receipts r JOIN purchase_lines l ON l.id=r.line_id WHERE l.invoice_id=? ORDER BY r.created_at',[invoice.id]).all()).results};
 }
 ...
 if(invoiceMatch&&method==='POST'){
  const [,key,action]=invoiceMatch,x=await readBody(request),existing=await statement(db,'SELECT status,supplier_id,invoice_date FROM purchase_invoices WHERE id=?',[key]).first();if(!existing)fail('Fatura bulunamadı.',404);
  if(action==='receive'){

...
export async function attentionApi(request,env,path){
 ...
 const queries=[
  ...
  `SELECT COALESCE(SUM(status='draft'),0) drafts,
   COALESCE(SUM(status='posted' AND EXISTS(SELECT 1 FROM purchase_lines l WHERE l.invoice_id=purchase_invoices.id AND l.line_type='product' AND l.quantity_milli>COALESCE((SELECT SUM(g.quantity_milli) FROM goods_receipts g WHERE g.line_id=l.id),0))),0) awaiting_receipt FROM purchase_invoices`,
  `SELECT COUNT(*) total,

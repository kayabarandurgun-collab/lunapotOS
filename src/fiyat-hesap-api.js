...
export async function fiyatHesapApi(request, env, path) {
  ...
  const urun = await db.prepare('SELECT p.id,p.name,pp.vat_bps,pp.replacement_cost_cents FROM ec_products p LEFT JOIN ec_price_profiles pp ON pp.product_id=p.id WHERE p.id=?').bind(q.get('product_id') || '').first();
  ...
  const h = (await kesintiTahmincisi(db))(channel, [{product_id: urun.id, quantity_milli: qty * 1000}]);
  ...
  const dokum = P => {
    ...
    return {fiyat: P, maliyet, kargo, hizmet, komisyon, stopaj, cebine: P - maliyet - kargo - hizmet - komisyon - stopaj};
  };
  const fiyatFor = hedef => degisken > 0 ? Math.ceil((hedef + maliyet + kargo + hizmet) / degisken) : null;
  return {

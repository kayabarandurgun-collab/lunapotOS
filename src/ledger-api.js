...
export async function ledgerApi(request,env,path,readBody){
 ...
 if(path==='/api/ledger'&&method==='GET'){
  ...
  const results=await db.batch([
   ...
   stmt(db,`SELECT e.id entry_id,iif(e.source_key LIKE 'invoice:%',substr(e.source_key,9),NULL) invoice_id,iif(e.source_key LIKE 'gecici:%',1,0) faturasiz,e.reference invoice_no,e.party_id,s.name party_name,e.occurred_on,-e.amount_cents debt_cents,${ALLOCATED('negative')} paid_cents,${PLANNED} planned_on,e.due_on FROM party_entries e JOIN suppliers s ON s.id=e.party_id WHERE ((e.source='invoice' AND e.source_key LIKE 'invoice:%') OR e.source_key LIKE 'gecici:%') AND e.amount_cents<0 AND ${LIVE}${party?' AND e.party_id=?':''} ORDER BY e.occurred_on,e.created_at,e.rowid LIMIT 501`,party?[party]:[]),

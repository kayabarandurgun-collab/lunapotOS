...
export function filterAccounting(x,user,ns){if(!user||user.owner)return x;if(ns==='lp')return can(user,ns,'accounts')?x:{...x,stock:[],sales:[],expenses:[],suppliers:[],invoices:[],movements:[],expense_schedules:[],pending_fee_cents:null};return {...x,stock:any(user,ns,['stock','sales','invoices'])?x.stock:[],sales:can(user,ns,'sales')?x.sales:[],expenses:can(user,ns,'expenses')?x.expenses:[],expense_schedules:can(user,ns,'expenses')?x.expense_schedules:[],suppliers:can(user,ns,'invoices')?x.suppliers.map(({balance_cents,purchase_cents,paid_cents,...s})=>s):[],invoices:can(user,ns,'invoices')?x.invoices:[],movements:can(user,ns,'stock')?x.movements:[],pending_fee_cents:any(user,ns,['reconciliation','performance'])?x.pending_fee_cents:null};}

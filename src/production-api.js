import {nextLotCode,LOT_PATTERN} from './lot-api.js';
...
export async function productionApi(request,env,path,readBody){
 ...
 try{
  ...
  if(path==='/api/production/jobs'){
   ...
   // İsteğe bağlı: bu üretimden parti (lot) açılır; üretimle AYNI işlemde yazılır. Parti stok değiştirmez.
   let lot=null;if(x.create_lot===true||x.create_lot==='on'){const code=x.lot_code?String(x.lot_code).trim():await nextLotCode(db,date,'lp_lots');if(!LOT_PATTERN.test(code))fail('Parti kodu 2-40 karakter olmalı ve harf/rakam ile başlamalı.');if(await db.prepare('SELECT id FROM lp_lots WHERE lot_code=?').bind(code).first())fail('Bu parti kodu zaten kullanılmış.',409);const best=x.best_before?(typeof x.best_before==='string'&&/^\d{4}-\d{2}-\d{2}$/.test(x.best_before)&&new Date(x.best_before+'T00:00:00Z').toISOString().slice(0,10)===x.best_before?x.best_before:fail('Son kullanma tarihi geçersiz.')):null;if(best&&best<date)fail('Son kullanma tarihi üretimden önce olamaz.');const pack=x.pack_size===undefined||x.pack_size===null||x.pack_size===''?null:qty(x.pack_size);const u=env.USER||{};lot={id:crypto.randomUUID(),code,best,pack,by:u.id||'',name:u.name||'Yönetici'};}
   await db.batch([
    ...
    ...(lot?[db.prepare('INSERT INTO lp_lots(id,lot_code,product_id,job_id,produced_on,best_before,quantity_milli,unit,pack_size_milli,note,created_by,created_by_name) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)').bind(lot.id,lot.code,recipe.product_id,jobId,date,lot.best,quantity,'adet',lot.pack,('Üretimden otomatik: '+String(x.reference||'')).slice(0,500),lot.by,lot.name)]:[])
   ]);return {id:jobId,total_cost_cents:total,lot_code:lot?lot.code:null};
  }

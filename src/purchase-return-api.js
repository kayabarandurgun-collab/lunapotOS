 ...
 if(match[2]==='returns'){
  ...
  for(const line of x.lines){
   ...
   statements.push(stmt(db,`INSERT INTO purchase_returns(id,line_id,quantity_milli,net_cents,tax_cents,cost_cents,operation_id,reference,occurred_on,reason)
    SELECT ?,l.id,?,CAST(ROUND(l.net_cents*(?+COALESCE((SELECT SUM(quantity_milli) FROM active_purchase_returns WHERE line_id=l.id),0))/(l.quantity_milli*1.0)) AS INTEGER)-COALESCE((SELECT SUM(net_cents) FROM active_purchase_returns WHERE line_id=l.id),0),CAST(ROUND(l.tax_cents*(?+COALESCE((SELECT SUM(quantity_milli) FROM active_purchase_returns WHERE line_id=l.id),0))/(l.quantity_milli*1.0)) AS INTEGER)-COALESCE((SELECT SUM(tax_cents) FROM active_purchase_returns WHERE line_id=l.id),0),CAST(ROUND(b.value_cents*1.0*?/MAX(b.quantity_milli,1.0)) AS INTEGER),?,?,?,? FROM purchase_lines l JOIN stock_balances b ON b.product_id=l.product_id WHERE l.id=?`,[id,quantity,quantity,quantity,quantity,operationId,reference,date,reason,line.id]));
  }
 }else if(match[3]){

...
export async function purchaseSplitApi(request,env,path,readBody){
 ...
 try{
  ...
  if(!Array.isArray(x.allocations)||x.allocations.length<1||x.allocations.length>20||new Set(x.allocations.map(a=>a?.product_id)).size!==x.allocations.length)fail('1–20 farklı stok ürünü seçin.');
  ...
  const id=crypto.randomUUID();await db.batch([db.prepare('INSERT INTO purchase_line_splits(id,invoice_id,original_line_id,original_json,allocations_json,reason,stock_unit,total_quantity_milli,family_id) VALUES(?,?,?,?,?,?,?,?,?)').bind(id,invoice.id,line.id,JSON.stringify(line),JSON.stringify(allocations),x.reason.trim(),products[0].stock_unit,total,family?family.id:null)]);
  return {id:invoice.id,split_id:id};
 }catch(e){if(e.status)throw e;if(/SPLIT_FAMILY/.test(e.message))fail('Seçilen çeşitlerden biri bu ürün ailesinin üyesi değil. Aileyi veya çeşitleri düzeltin.',409);
  if(/SPLIT_UNIT/.test(e.message))fail('Çeşitler aynı stok biriminde olmalı.',409);
  if(/SPLIT_|IMMUTABLE_|UNIQUE constraint/.test(e.message))fail('Dağılım kaydedilemedi: fatura değişmiş, dağıtılmış veya kilitlenmiş olabilir. Faturayı yenileyin.',409);throw e;}
}

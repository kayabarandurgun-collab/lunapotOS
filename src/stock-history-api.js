...
export async function stockHistoryApi(request,env,path){
 ...
 const cte=`WITH history AS (SELECT id,quantity_milli,value_cents,kind,reference,notes,occurred_on,created_at,${ec?"'manual'":'origin'} origin FROM stock_movements WHERE product_id=?${ec?` UNION ALL SELECT 'adjustment:'||a.id,0,iif(a.reversal_of IS NULL,-a.stock_cents,a.stock_cents),'adjustment',a.reference,a.reason,a.occurred_on,a.created_at,'adjustment' FROM purchase_adjustments a JOIN purchase_lines l ON l.id=a.line_id WHERE l.product_id=? AND a.stock_cents!=0 UNION ALL SELECT 'settlement:'||c.id,0,-c.value_cents,'cost_settlement',c.source_id,'Stokta olmadan satılan malın maliyeti bu girişle kapandı',c.occurred_on,c.created_at,'settlement' FROM cost_settlements c WHERE c.product_id=?`
  // FIFO düzeltmeleri ve geçici sayım kapanış farkları da stok değerini değiştirir: dökümün toplamı bakiyeyle tutsun.
  +` UNION ALL SELECT 'revaluation:'||v.id,0,-v.delta_cents,'cost_revaluation',v.sale_id,'Satış maliyeti tarih sırasıyla (FIFO) düzeltildi',substr(v.created_at,1,10),v.created_at,'revaluation' FROM ec_cost_revaluations v WHERE v.product_id=?`
  +` UNION ALL SELECT 'close:'||r.id,0,-r.value_cents,'cost_revaluation',r.movement_id,iif(r.kind='kayip','Geçici sayımda kaybolan malın fiyat farkı','Geçici sayım faturanın gerçek fiyatına çekildi'),substr(r.created_at,1,10),r.created_at,'revaluation' FROM ec_close_cost_revaluations r WHERE r.product_id=?`:''}) `;
 const values=[product,...(ec?[product,product,product,product]:[]),...args];

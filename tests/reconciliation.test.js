...
function fixture(){
 ...
 const call=(workspace,path='',body)=>{const DB={...raw,prepare(query){for(const table of ['purchase_line_limits','fee_allocations','fee_audit','purchase_invoices','purchase_lines','sale_entries','suppliers'])query=query.replace(new RegExp('\\b'+table+'\\b','g'),workspace+'_'+table);if(workspace==='ec')query=query.replace(/\bproducts\b/g,'ec_products');return raw.prepare(query);}};return reconciliationApi(new Request('https://test.local/api/reconciliation'+path,{method:body?'POST':'GET'}),{DB,WORKSPACE:workspace},'/api/reconciliation'+path,async()=>body);};
 function seed(ns='ec'){
